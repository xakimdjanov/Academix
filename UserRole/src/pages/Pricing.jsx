    ...
    <div className="bg-[#F6F8FB] min-h-screen pb-20">
      ...
      <section className="bg-[#002147] text-white py-12 md:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl md:text-6xl font-black mb-6 tracking-tight">Oddiy va shaffof tariflar</h1>
          <p className="text-blue-100/70 text-lg md:text-xl max-w-2xl mx-auto font-medium">
            Akademik nashrlar uchun eng maqbul yechimlar. Yashirin to'lovlar va murakkabliklarsiz.
          </p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredTariffs.map((tariff, idx) => (
              <PricingCard 
                ...
                price={tariff.price} 
                token={token}
                desc={tariff.description || `${tariff.type === 'Article' ? 'Maqola' : 'Jurnal'} uchun maxsus tarif.`}
                features={[
                  tariff.type === 'Journal' ? (tariff.journal_limit ? `${tariff.journal_limit} ta jurnal` : "Cheksiz jurnallar") : null,
                  tariff.article_limit ? `${tariff.article_limit} ta maqola` : "Cheksiz maqolalar",
                  tariff.duration_days ? `${tariff.duration_days} kunlik muddat` : "Umrbod foydalanish",
                  ...(tariff.description ? tariff.description.split(/[\n,]+/).map(f => f.trim()).filter(f => f.length > 0) : [])
                ].filter(Boolean)}
                notFeatures={idx === 0 ? ["DOI biriktirish", "Ustuvor qo'llab-quvvatlash"] : []}
                popular={idx === 1}
              />
            ))}
            ...
            {filteredTariffs.length === 0 && (
                <div className="col-span-full bg-white p-16 rounded-[3rem] text-center shadow-sm border border-gray-100">
                    ...
                    <p className="text-gray-400 font-bold text-lg">Bu bo'limda hozircha tariflar mavjud emas.</p>
                </div>
            )}
          </div>
        )}
      ...
      <section className="py-20 bg-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
           <h2 className="text-3xl md:text-4xl font-black text-[#002147] mb-16 text-center">Imkoniyatlarni solishtirish</h2>
              ...
              <table className="w-full text-left border-collapse">
                 <thead>
                    <tr className="bg-gray-50 text-[10px] font-black uppercase text-gray-400 tracking-widest border-b border-gray-100">
                       <th className="px-8 py-6">Xususiyatlar</th>
                       <th className="px-8 py-6 text-center">Boshlang'ich</th>
                       <th className="px-8 py-6 text-center">Professional</th>
                       <th className="px-8 py-6 text-center">Korporativ</th>
                    </tr>
                 </thead>
                 <tbody className="text-sm">
                    <ComparisonRow title="Maqolalar limiti" free="Cheklangan" pro="Cheksiz" enterprise="Cheksiz" />
                    ...
                    <ComparisonRow title="Qo'llab-quvvatlash" free="Email" pro="24/7 Chat" enterprise="Shaxsiy menejer" />
                 </tbody>
              </table>
           </div>
        </div>
...
const PricingCard = ({ id, icon, title, price, desc, features, notFeatures, popular, token }) => (
  <div className={`p-10 rounded-[3rem] border-2 transition-all flex flex-col h-full bg-white group hover:scale-[1.02] duration-500 ${popular ? 'border-blue-600 shadow-2xl relative z-10 scale-105' : 'border-gray-50 shadow-sm shadow-blue-900/5 hover:border-blue-200'}`}>
    {popular && <div className="absolute top-0 right-12 -translate-y-1/2 bg-blue-600 text-white text-[10px] font-black uppercase px-6 py-2 rounded-full tracking-tighter">Eng ko'p tanlangan</div>}
    ...
    <div className="flex items-baseline mb-4">
       ...
       <span className="text-sm font-bold text-gray-400 ml-2">/ yakuniy</span>
    </div>
    ...
    >
       {token ? "PANELGA O'TISH" : "HOZIROQ BOSHLASH"}
    </Link>
  </div>
);
